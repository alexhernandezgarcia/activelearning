...
class Querier:
        ...
        if self.config.al.query_mode == "random":
            ...
            if self.comet:
                self.comet.log_histogram_3d(
                    self.sampleDict["scores"],
                    name="sampler output scores",
    ...
    def constructQuery(self, samples, scores, uncertainties, nQueries):
        ...
        while (
            ...
            samples = filterDuplicateSamples(
                np.concatenate((samples, randomSamples), axis=0)
            )
    ...
    def sampleForQuery(self, model, iterNum):
        ...
        sampleDict = self.runSampling(
            model, scoreFunction=scoreFunction, al_iter=iterNum
        )
    ...
    def getScoreFunction(self):
        if self.config.al.query_mode == "energy":
        ...
        elif (self.config.al.query_mode == "heuristic") or (
            self.config.al.query_mode == "learned"
        ):
            ...
            raise ValueError(
                self.config.al.query_mode + "is not a valid query function!"
            )
    ...
    def runSampling(
        ...
        if method.lower() == "mcmc":
            ...
            if self.config.al.query_mode == "fancy_acquisition":
                scores, energies, std_dev = model.evaluate(
                    samples, output="fancy_acquisition"
                )
            else:
                energies, std_dev = model.evaluate(samples, output="Both")
                scores = energies * scoreFunction[0] - scoreFunction[1] * np.asarray(
                    std_dev
                )
            outputs = {
                "samples": samples,
            ...
            printRecord(
                "Random sampling and annealing took {} seconds".format(
                    int(time.time() - t0)
                )
            )
            ...
            outputs, times = gflownet.sample_batch(
                gflownet.env, self.config.gflownet.n_samples, train=False
            )
            outputs, times_batch = batch2dict(
                outputs,
                gflownet.env,
    ...
    def doAnnealing(self, scoreFunction, model, outputs, seed=0, useOracle=False):
        ...
        annealedOutputs = annealer.postSampleAnnealing(
            ...
            model,
            useOracle=useOracle,
            seed=self.config.seeds.sampler + seed,
        )
