...
IS_CPU=0
IS_CUDA=0
ENV_PATH="activelearning-env"
EXTRAS="dev"
DRY_RUN=0

# Allowed extras set
ALL_EXTRAS=("dev" "materials")
...
for arg in "$@"; do
    if [[ "$arg" == "--help" ]]; then
		...
		echo "  --envpath PATH        Path of the Python virtual environment to be installed. Default: ./activelearning-env"
		...
		echo ""
		echo "Example:"
		echo "    source install.sh --cpu --envpath ~/myenvs/activelearning --extras dev,materials"
		return 0
    fi
done
...
# Exit
echo "Installation of activelearning package completed!"
