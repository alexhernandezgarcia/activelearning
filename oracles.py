...
def toyHamiltonian(queries):
    ...
    :return:
    """
    hamiltonian = np.random.random(
        (queries.shape[1], queries.shape[1])
    )  # energy function
    hamiltonian = (
    ...
    for i in range(len(queries)):
        energies[i] = (
            queries[i] @ hamiltonian @ queries[i].transpose()
...
def PottsEnergy(queries, nalphabet=4):
    ...
    pottsH = np.random.random(
        (queries.shape[1], nalphabet)
    ...
    for k in range(len(queries)):
        ...
        for ii in range(nnz):  # ignore padding terms
            ...
            for jj in range(
                ...
                energies[k] += (
                    2 * pottsJ[ii, jj, queries[k, ii] - 1, queries[k, jj] - 1]
