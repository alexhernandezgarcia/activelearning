...
class ActiveLearning:
    def __init__(self, config):
        ...
        with open(self.workDir + "/config.yml", "w") as f:
            yaml.dump(
                numpy2python(namespace2dict(self.config)), f, default_flow_style=False
            )
    ...
    def runPipeline(self):
        ...
        for _ in range(self.config.al.episodes):
            ...
            self.testMinima = (
                []
            )  # best test loss of models, for each iteration of the pipeline
            self.bestScores = (
                []
    ...
    def iterate(self):
        ...
        self.getModelState(
            self.terminal
        )  # run energy-only sampling and create model state dict
        self.getDatasetReward()
        printRecord(
            "Model state calculation took {} seconds".format(int(time.time() - t0))
        )
        ...
        if self.terminal == 0:  # skip querying if this is our final pipeline iteration
            ...
            query = self.querier.buildQuery(
                self.model, self.stateDict, action=self.action, comet=self.comet
            )  # pick Samples to be scored
            printRecord(
                "Query generation took {} seconds".format(int(time.time() - t0))
            )
            ...
            if self.comet:  # report query scores to comet
                self.comet.log_histogram_3d(
                    energies, name="query energies", step=self.pipeIter
                )
    ...
    def getModelState(self, terminal):
        ...
        if terminal:  # use the query-generating sampler for terminal iteration
            ...
            sampleDict = self.querier.runSampling(
                self.model,
                scoreFunction=[1, 0],
                al_iter=self.pipeIter,
                method_overwrite="random",
        ...
        energies = sampleDict["energies"][sort_inds][
            : self.config.querier.model_state_size
        ]
        ...
        printRecord(
            ...
            + bcolors.OKGREEN
            + " {:.2f},".format(
                np.average(energies[: self.config.querier.model_state_size])
            )
            + bcolors.ENDC
            + " and average std dev"
            + bcolors.OKCYAN
        ...
        if self.comet:
            ...
            self.comet.log_histogram_3d(
                datasetDist,
                name="model state distance from dataset",
                step=self.pipeIter,
            )
            self.comet.log_histogram_3d(
                randomDist,
                name="model state distance from large random sample",
                step=self.pipeIter,
            )
            self.comet.log_histogram_3d(
    ...
    def getModelStateReward(self, bestEns, bestStdDevs):
        ...
        best_adjusted_energy = np.amin(
            adjusted_energies
        )  # best energy, adjusted for uncertainty
        if self.pipeIter == 0:
        ...
        if (
            ...
            if self.pipeIter == 0:
                self.model_state_abs_score = [
                    1
                    - np.abs(self.trueMinimum - best_adjusted_energy)
                    / np.abs(self.trueMinimum)
                ]
        ...
        if (
            ...
            if self.pipeIter == 0:
                self.dataset_abs_score = [
                    1
                    - np.abs(self.trueMinimum - best_energy) / np.abs(self.trueMinimum)
                ]
                ...
                self.dataset_normed_cumulative_score = (
                    self.dataset_cumulative_score / xaxis[-1]
                )
                printRecord(
    ...
    def largeModelEvaluation(self):
        ...
        numSamples = min(
            int(1e3),
            self.config.dataset.dict_size ** self.config.dataset.max_length // 100,
        ...
        bottomTenLoss = F.mse_loss(
            (torch.Tensor(modelScores[:bestTenInd]).float() - self.model.mean)
            / self.model.std,
            (torch.Tensor(randomScores[:bestTenInd]).float() - self.model.mean)
            / self.model.std,
    ...
    def runPureSampler(self):
        ...
        if self.config.al.sample_method == "mcmc":
            ...
            mcmcSampler = Sampler(
                self.config, self.config.seeds.sampler, [1, 0], gammas
            )
            sampleDict = mcmcSampler.sample(
                self.model, useOracle=True
            )  # do a genuine search
            ...
            sampleDict = self.querier.doAnnealing(
                [1, 0], self.model, outputs, useOracle=True
            )
            ...
            gflownet.train()
            printRecord(
                "Training GFlowNet took {} seconds".format(int(time.time() - t0))
            )
            t0 = time.time()
            sample_batch, times = gflownet.sample_batch(
                gflownet.env, self.config.gflownet.n_samples, train=False
            )
            sampleDict, times = batch2dict(
                sample_batch,
                gflownet.env,
                get_uncertainties=False,
                query_function=self.config.al.query_mode,
            )
            printRecord(
        ...
        clusterDict = {
            "energies": np.asarray(
                [np.amin(cluster_scores) for cluster_scores in clusterScores]
            ),
            "samples": np.asarray(
        ...
        global_minimum = min(
            np.amin(sampleDict["energies"]), self.getTrueMinimum(sampleDict)
        )
        ...
        if self.comet:
            ...
            self.comet.log_metric(
                "Best sample",
                numbers2letters(
                    sampleDict["samples"][np.argmin(sampleDict["energies"])]
                ),
            )
        ...
        print(
            "Top K Cluster Energies {:.3f} {:.3f} {:.3f}".format(
                top_cluster_energies[0],
                top_cluster_energies[1],
                top_cluster_energies[2],
            )
        )
        print(
    ...
    def sampleOracle(self):
        ...
        if (self.config.dataset.oracle == "linear") or (
        ...
        if self.comet:
            self.comet.log_histogram_3d(
                sampleDict["energies"], name="energies_true", step=0
            )
    ...
    def saveOutputs(self):
        ...
        if self.config.dataset.type == "toy":
            ...
            if self.pipeIter > 1:
                ...
                outputDict["model state cumulative score"] = (
                    self.model_state_cumulative_score,
                )
                outputDict[
                    "model state per sample cumulative score"
                ] = self.model_state_normed_cumulative_score
                outputDict["dataset score record"] = self.dataset_abs_score
                outputDict["dataset cumulative score"] = (
                    self.dataset_cumulative_score,
                )
                outputDict[
    ...
    def updateDataset(self, oracleSequences, oracleScores):
        ...
        printRecord(
            f"Added{bcolors.OKBLUE}{bcolors.BOLD} %d{bcolors.ENDC}"
            % int(len(oracleSequences))
        ...
        np.save(
            "datasets/" + self.config.dataset.oracle + "_iter_{}".format(self.pipeIter),
            dataset,
        )
    ...
    def logTopK(self, dataset, prefix, returnScores=False):
        if self.comet:
            ...
            for k in [1, 10, 100]:
                ...
                dist = binaryDistance(
                    topk_samples, pairwise=False, extractInds=len(topk_samples)
                )
                self.comet.log_metric(
                    prefix + f" mean top-{k} energies",
                    np.mean(topk_scores),
                    step=self.pipeIter,
                )
                self.comet.log_metric(
                    prefix + f" std top-{k} energies",
                    np.std(topk_scores),
                    step=self.pipeIter,
                )
                self.comet.log_metric(
    ...
    def addRandomSamples(
        ...
        rands = np.random.randint(
            0,
            len(samples),
            size=self.config.querier.model_state_size - len(minClusterSamples),
        )
        ...
        printRecord(
            "Padded model state with {} random samples from sampler run".format(
                len(rands)
            )
        )
