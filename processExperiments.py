...
def getScores(experiment, experiments_dir):
    ...
    for run in runs:
        run_outputs.append(
            np.load(run + "/episode0/outputsDict.npy", allow_pickle=True).item()
        )
    ...
    cumulative_score = np.asarray(
        [run_outputs[i]["cumulative score"] for i in range(len(runs))]
    )
    dists = np.asarray(
        [
            [
                [
                    run_outputs[i]["state dict record"][j][
                        "best clusters internal diff"
                    ],
                    run_outputs[i]["state dict record"][j][
                        "best clusters dataset diff"
                    ],
                    run_outputs[i]["state dict record"][j][
                        "best clusters random set diff"
                    ],
                ]
...
for i in range(len(scoreList)):
    plt.subplot(1, 3, 1)
    plt.plot(
        np.arange(1, 21), np.average(scoreList[i], axis=0), "o-", label=experiments[i]
    )
    ...
    plt.plot(
        np.arange(1, 21),
        np.average(rewardsList[i], axis=0).transpose(),
        "o-",
        label=experiments[i],
    )
    plt.ylabel("Per-Iteration Reward", size=24)
    ...
    plt.plot(
        np.arange(1, 21),
        np.average(distList[i, :, :, 2, :], axis=(1, -1)),
        "o-",
        label=experiments[i],
    )
    plt.subplot(4, 3, 6)
    ...
    plt.plot(
        np.arange(1, 21),
        np.average(distList[i, :, :, 1, :], axis=(1, -1)),
        "o-",
        label=experiments[i],
    )
    plt.subplot(4, 3, 9)
    plt.plot(
        np.arange(1, 21),
        np.average(distList[i, :, :, 0, :], axis=(1, -1)),
        "o-",
        label=experiments[i],
    )
    plt.xlabel("AL Iteration", size=24)
