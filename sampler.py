...
class Sampler:
    ...
    def __init__(self, config, seedInd, scoreFunction, gammas):
        ...
        self.randintsResampleAt = int(
            1e4
        )  # larger takes up more memory but increases speed
    ...
    def makeAConfig(self):
        ...
        if (
            self.config_main.dataset.variable_length
            and self.config_main.dataset.min_length
            < self.config_main.dataset.max_length
        ):
            ...
            if (
                randChainLen < self.config_main.dataset.max_length
    ...
    def resampleRandints(self):
        ...
        self.spinRandints = np.random.randint(
            1,
            self.config_main.dataset.dict_size + 1,
            size=(self.nruns, self.randintsResampleAt),
        ).astype("uint8")
        self.pickSpinRandint = np.random.randint(
            0, self.chainLength, size=(self.nruns, self.randintsResampleAt)
        ).astype("uint32")
        self.alphaRandoms = np.random.random(
            (self.nruns, self.randintsResampleAt)
        ).astype(float)
    ...
    def initOptima(self, scores, energy, std_dev):
        ...
        self.all_energies = np.zeros_like(
            self.all_scores
        )  # record energies near the optima
    ...
    def converge(self, model, useOracle=False, nIters=False):
        ...
        for self.iter in tqdm.tqdm(
            ...
            if (
                self.iter % self.randintsResampleAt == 0
            ):  # periodically resample random numbers
                self.resampleRandints()

        printRecord(
            "{} samples were recorded on this run".format(
                len(np.concatenate(self.all_samples))
            )
        )
    ...
    def postSampleAnnealing(self, initConfigs, model, useOracle=False, seed=0):
        ...
        self.temp0 = (
            0.01  # initial temperature for sampling runs - start low and shrink
        )
        self.temperature = [self.temp0 for _ in range(self.nruns)]
    ...
    def getScores(self, propConfig, config, model, useOracle):
        ...
        if useOracle:
            ...
            std_dev = [
                [0 for _ in range(len(energy[0]))],
                [0 for _ in range(len(energy[1]))],
            ]
            score = self.scoreFunction[0] * np.asarray(energy) - self.scoreFunction[
                1
            ] * np.asarray(
                std_dev
            ...
            if self.config_main.al.query_mode == "fancy_acquisition":
                score1, out1, std1 = model.evaluate(
                    np.asarray(config), output="fancy_acquisition"
                )
