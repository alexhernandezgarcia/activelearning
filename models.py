...
class modelNet:
    ...
    def initModel(self):
        ...
        if (
            ...
            self.config.proxy.model_type == "transformer2"
        ):  # upgraded self-attention model
    ...
    def save(self, best):
        if best == 0:
            torch.save(
                ...
                "ckpts/" + getModelName(self.ensembleIndex) + "_final",
            )
        elif best == 1:
            torch.save(
                {
                ...
                "ckpts/" + getModelName(self.ensembleIndex),
            )
        ...
        if os.path.exists("ckpts/" + dirName):  # reload model
            ...
            if (
                ...
                for i in list(checkpoint["model_state_dict"]):
                    checkpoint["model_state_dict"][i[7:]] = checkpoint[
                        "model_state_dict"
                    ].pop(i)
    ...
    def getLoss(self, train_data):
        ...
        targets = (
            targets - self.mean
        ) / self.std  # standardize the targets during training
        ...
        if output == "Average":
            ...
            if self.config.al.acquisition_function.lower() == "ucb":
                ...
                try:
                    if (
                        self.best_f == "canoe"
                    ):  # I just want it to load for goodness sake
                        pass
                except:
                    self.getMinF()
...
class buildDataset:
    ...
    def __init__(self, config):
        dataset = np.load(
            "datasets/" + config.dataset.oracle + ".npy", allow_pickle=True
        )
    ...
    def reshuffle(self, seed=None):
        self.samples, self.targets = shuffle(
            self.samples, self.targets, random_state=seed
        )
...
def getDataloaders(
    config, ensembleIndex
):  # get the dataloaders, to load the dataset in batches
    """
...
class transformer(nn.Module):
    def __init__(self, config):
        ...
        self.positionalEncoder = PositionalEncoding(
            self.embedDim, max_len=self.maxLen, dropout=0
        )
    ...
    def forward(self, x):
        ...
        for i in range(len(self.self_attn_layers)):
            x = self.self_attn_layers[i](x, x, x, key_padding_mask=x_key_padding_mask)[
                0
            ]
        ...
        for i in range(self.encoder_layers):
            ...
            if not self.relative_attention:
                x = self.self_attn_layers[i](
                    x, x, x, key_padding_mask=x_key_padding_mask
                )[0]
            else:
